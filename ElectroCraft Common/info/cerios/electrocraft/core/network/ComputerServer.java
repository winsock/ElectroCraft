...
public class ComputerServer implements Runnable {
	...
	public ComputerServer(int port) throws IOException {
		...
		ElectroCraft.instance.getLogger()
				.info("ComputerServer: Running on port " + String.valueOf(port)
						+ "!");
	}
	...
	public void run() {
		while (running) {
			try {
				...
				ComputerServerClient client = new ComputerServerClient(this,
						connection);
				List players = FMLCommonHandler.instance()
				...
				for (int i = 0; i < players.size(); i++) {
					...
					if (((InetSocketAddress) player.playerNetServerHandler.netManager
							.getSocketAddress()).getHostName()
							.equalsIgnoreCase(
									connection.getInetAddress().getHostName())) {
						ElectroCraft.instance.getLogger().info(
								"ComputerServer: Client Connected!");
