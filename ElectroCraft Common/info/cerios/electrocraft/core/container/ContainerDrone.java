...
public class ContainerDrone extends Container {
	...
	public ContainerDrone(InventoryDrone inventory,
			InventoryPlayer playerInventory) {

		// Drone inv
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 9; j++) {
				addSlotToContainer(new Slot(inventory, (i * 9) + j, 8 + j * 18,
						26 + i * 18));
			}
		}
		...
		for (int j = 0; j < 3; j++) {
			for (int i1 = 0; i1 < 9; i1++) {
				addSlotToContainer(new Slot(playerInventory, i1 + j * 9 + 9,
						8 + i1 * 18, 84 + j * 18));
			}
		}
	...
	public ItemStack func_82846_b(EntityPlayer player, int slot) {
		...
		if (slotObject != null && slotObject.getHasStack()) {
			...
			if (slotObject.inventory instanceof InventoryDrone) {
				if (!mergeItemStack(stackInSlot, 40, inventorySlots.size(),
						true))
					return null;
