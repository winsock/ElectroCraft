...
public class SwordTool implements IDroneTool {
	...
	@Override
	public List<ItemStack> preformAction(ItemStack item, EntityDrone drone,
			World world, int x, int y, int z) {
		List<ItemStack> itemsDropped = new ArrayList<ItemStack>();

		Vec3 look = drone.getLookVec();
		// field_82592_a = getVecFromPool
		Vec3 pos = Vec3.field_82592_a.getVecFromPool(drone.posX, drone.posY,
				drone.posZ);
		Vec3 vector = pos.addVector(look.xCoord * 4, look.yCoord * 4,
				look.zCoord * 4);

		Entity entity = null;
		List entites = world.getEntitiesWithinAABBExcludingEntity(
				drone,
				drone.boundingBox.addCoord(look.xCoord, look.yCoord,
						look.zCoord).expand(1.0D, 1.0D, 1.0D));
		double distance = 0.0D;
		Iterator entityIterator = entites.iterator();
		while (entityIterator.hasNext()) {
			Entity e = (Entity) entityIterator.next();
			float expandOffset = 0.3F;
			AxisAlignedBB boudingBoxToSearch = e.boundingBox.expand(
					expandOffset, expandOffset, expandOffset);
			MovingObjectPosition var12 = boudingBoxToSearch.calculateIntercept(
					pos, vector);
			if (var12 != null) {
		...
		if (entity != null) {
			...
			if (rayTrace != null
					&& rayTrace.typeOfHit == EnumMovingObjectType.ENTITY
					&& rayTrace.entityHit != null) {
				rayTrace.entityHit.captureDrops = true;
				rayTrace.entityHit.capturedDrops.clear();
				rayTrace.entityHit.attackEntityFrom(
						DamageSource.causeMobDamage(drone),
						item.getDamageVsEntity(rayTrace.entityHit));
	...
	@Override
	public void damageItem(EntityDrone drone, ItemStack item, Block block,
			int metadata) {
	}
}
