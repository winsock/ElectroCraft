...
public class ComputerFile implements LuaAPI {
	...
	public ComputerFile(String pathname, Computer computer) {
		...
		if (!pathname.startsWith("/rom")) {
			this.javaFile = new File(computer.getBaseDirectory()
					+ File.separator + pathname);

			try {
				if (!Utils.baseDirectoryContains(computer.getBaseDirectory(),
						javaFile))
					throw new LuaRuntimeException(
							"Error! Not allowed to open files outside of its base directory!");
			} catch (IOException e) {
				throw new LuaRuntimeException("Error checking if path is valid");
			}

			if (pathname.contains(".persist"))
				throw new LuaRuntimeException(
						"Error! Not allowed to open the persist file!");

			if (!this.exists()) {
				if (computer.getBaseDirectory().length() > ConfigHandler
						.getCurrentConfig()
						.get("computer", "MaxStoragePerUser", 10).getInt(10) * 1024 * 1024)
					throw new LuaRuntimeException(
							"Error! Tried to make a new file when the disk is full!");
			}

			if (computer.getNumberOfOpenFileHandles() >= computer
					.getMaxFileHandles())
				throw new LuaRuntimeException(
						"Error! Tried to open too many files!");
			else {
				computer.incrementOpenFileHandles();
			}
		} else {
			resource = this.getClass().getResourceAsStream(
					"/info/cerios/electrocraft" + pathname);
		}
	...
	public boolean canWrite() {
		if (resource != null)
			return false;
		if (computer.getBaseDirectory().length() > ConfigHandler
				.getCurrentConfig().get("computer", "MaxStoragePerUser", 10)
				.getInt(10) * 1024 * 1024)
			return false;
	...
	public long getFreeSpace() {
		...
		return (ConfigHandler.getCurrentConfig()
				.get("computer", "MaxStoragePerUser", 10).getInt(10) * 1024 * 1024)
				- computer.getBaseDirectory().length();
	}
	...
	public ComputerFile getParentFile() {
		if (resource != null)
			return new ComputerFile(pathname.endsWith("/") ? pathname
					.substring(0, pathname.length() - 1).substring(
							0,
							pathname.substring(0, pathname.length() - 1)
									.lastIndexOf("/")) : pathname.substring(0,
					pathname.lastIndexOf("/")), computer);
		try {
			if (!Utils.baseDirectoryContains(computer.getBaseDirectory(),
	...
	public Object open(String mode) throws IOException {
		if (resource != null) {
			if (mode.equalsIgnoreCase("r"))
				return new ComputerBufferedReader(new InputStreamReader(
						resource));
			...
			return new ComputerOutputStream(
					new FileOutputStream(javaFile, true));
		return null;
	}
	...
	public NamedJavaFunction[] getGlobalFunctions(Computer computer) {
		return new NamedJavaFunction[] { new NamedJavaFunction() {
			...
			public int invoke(LuaState luaState) {
				try {
					ComputerFile file = new ComputerFile(
							luaState.checkString(-1), computer);
